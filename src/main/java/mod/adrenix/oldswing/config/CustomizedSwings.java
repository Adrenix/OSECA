...
public abstract class CustomizedSwings
{
    ...
    {
        return ForgeRegistries.ITEMS.getValue(ResourceLocation.tryParse(entry.getKey()));
    }
    ...
    public static Map.Entry<String, Integer> getEntryFromItem(Item item)
    {
        List<Map.Entry<String, Integer>> entries = new ArrayList<>(config.custom.entrySet());
        for (Map.Entry<String, Integer> entry : entries)
            if (entry.getKey().equals(Objects.requireNonNull(ForgeRegistries.ITEMS.getKey(item)).toString()))
                return entry;
        return null;
    }
